...
class ExtractedSupportTicket(BaseModel):
    ...
    category: TicketCategory = Field(
        description="Assigned category of the ticket"
    )
    ...
    one_line_summary: str = Field(
        description="A concise single-sentence summary of the user issue",
        max_length=150,
    )
    ...
    def must_be_single_line(cls, v: str) -> str:
        ...
        if "\n" in v or "\r" in v:
            raise ValueError(
                "Summary must be a single line without line breaks."
            )
